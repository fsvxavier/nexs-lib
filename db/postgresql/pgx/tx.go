...
// Transaction implementa a interface common.ITransaction usando pgx
...
func (t *Transaction) SendBatch(ctx context.Context, batch common.IBatch) (common.IBatchResults, error) {
	...
	br := t.tx.SendBatch(ctx, pgxBatch)
...
func (t *Transaction) Ping(ctx context.Context) error {
	// No pgx, não há um método Ping específico para transações
