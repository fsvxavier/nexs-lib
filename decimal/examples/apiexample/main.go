...
func main() {
	fmt.Println("=== EXEMPLOS DE USO DA API DECIMAL ===")
	fmt.Println("======================================")

	// Exemplo com ShopSpring usando Factory
	fmt.Println("\n=== Usando ShopSpring Provider via Factory ===")
...
func helpersAPIExample() {
	...
	jsonData1, _ := json.Marshal(dec4)
	fmt.Printf("JSON ShopSpring: %s\n", string(jsonData1))

	jsonData2, _ := json.Marshal(dec5)
	...
	// Verificar antes para evitar pânico
	if zero.IsZero() {
		fmt.Println("Divisão por zero detectada e evitada")
	} else {
		_, err = num.Div(zero)
		if err != nil {
