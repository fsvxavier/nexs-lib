...
func TestNewServer(t *testing.T) {
	...
	if !ok {
		t.Fatal("Expected server to be of type *atreugo.Server")
	}

	if atreugoServer.GetAtreugoApp() == nil {
		t.Error("Expected Atreugo app to be initialized")
	}
}
...
func TestServerSetHandler(t *testing.T) {
	...
	atreugoServer := server.(*Server)
	if atreugoServer.handler == nil {
		t.Error("Expected handler to be set")
	}
}

func TestServerGetAddr(t *testing.T) {
	cfg := config.DefaultConfig().WithHost("localhost").WithPort(9093)
	server, _ := NewServer(cfg)
...
func TestServerGetHTTPServer(t *testing.T) {
	...
	if httpServer != nil {
		t.Error("Expected nil HTTP server for Atreugo implementation")
	}
}
...
func TestFactory(t *testing.T) {
	...
	_, ok := server.(*Server)
	if !ok {
		t.Error("Expected server to be of type *atreugo.Server")
	}
}
