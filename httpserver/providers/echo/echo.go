...
// Factory implements the Factory pattern for creating Echo HTTP servers.
type Factory struct{}

// Server represents an Echo HTTP server implementation.
type Server struct {
	echo       *echo.Echo
	config     *config.BaseConfig
	observers  *hooks.ObserverManager
	middleware []echo.MiddlewareFunc
	routes     map[string]map[string]echo.HandlerFunc // method -> path -> handler
	mutex      sync.RWMutex
	running    int32
...
// Create creates a new Echo server instance.
func (f *Factory) Create(cfg interface{}) (interfaces.HTTPServer, error) {
	...
	for _, method := range supportedMethods {
		server.routes[method] = make(map[string]echo.HandlerFunc)
	}
	...
	for _, middleware := range baseConfig.GetMiddlewares() {
		// Skip middleware registration here as they need to be Echo-specific
		_ = middleware
	}
...
// GetDefaultConfig returns the default configuration for Echo.
func (f *Factory) GetDefaultConfig() interface{} {
	return config.NewBaseConfig()
}

// ValidateConfig validates the configuration for Echo.
...
func (s *Server) Start(ctx context.Context) error {
	...
	s.echo.Server.ReadTimeout = s.config.GetReadTimeout()
	s.echo.Server.WriteTimeout = s.config.GetWriteTimeout()
	s.echo.Server.IdleTimeout = s.config.GetIdleTimeout()
...
// Stop stops the Echo HTTP server gracefully.
...
func (s *Server) RegisterRoute(method, path string, handler interface{}) error {
	...
	echoHandler, ok := handler.(echo.HandlerFunc)
	if !ok {
		return fmt.Errorf("handler must be an echo.HandlerFunc, got %T", handler)
	}
	...
	// Store route
	methodMap[path] = echoHandler

	// Register route with Echo
	switch strings.ToUpper(method) {
	case "GET":
		s.echo.GET(path, s.wrapHandler(method, path, echoHandler))
	case "POST":
		s.echo.POST(path, s.wrapHandler(method, path, echoHandler))
	case "PUT":
		s.echo.PUT(path, s.wrapHandler(method, path, echoHandler))
	case "DELETE":
		s.echo.DELETE(path, s.wrapHandler(method, path, echoHandler))
	case "PATCH":
		s.echo.PATCH(path, s.wrapHandler(method, path, echoHandler))
	case "HEAD":
		s.echo.HEAD(path, s.wrapHandler(method, path, echoHandler))
	case "OPTIONS":
		s.echo.OPTIONS(path, s.wrapHandler(method, path, echoHandler))
	default:
		return fmt.Errorf("unsupported HTTP method: %s", method)
	}
...
// RegisterMiddleware registers a middleware with the Echo server.
func (s *Server) RegisterMiddleware(middleware interface{}) error {
	...
	if !ok {
		return fmt.Errorf("middleware must be an echo.MiddlewareFunc, got %T", middleware)
	}

	s.mutex.Lock()
	defer s.mutex.Unlock()

	s.middleware = append(s.middleware, echoMiddleware)
...
// wrapHandler wraps an Echo handler with observability and statistics.
func (s *Server) wrapHandler(method, path string, handler echo.HandlerFunc) echo.HandlerFunc {
...
// NewFactory creates a new Echo factory.
