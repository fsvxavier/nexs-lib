...
func TestNewFactory(t *testing.T) {
	...
	if factory.GetName() != "echo" {
		t.Errorf("Expected name 'echo', got '%s'", factory.GetName())
	}
}
...
func TestServerGetStats(t *testing.T) {
	...
	if stats.Provider != "echo" {
		t.Errorf("Expected provider 'echo', got '%s'", stats.Provider)
	}
...
func TestServerStartStop(t *testing.T) {
	factory := NewFactory()
	cfg, err := config.NewBuilder().Apply(config.WithPort(8093)).Build()
	if err != nil {
...
func TestServerHandlerWrapping(t *testing.T) {
	...
	if echoServer.routes["GET"]["/test"] == nil {
...
func TestServerWithMiddleware(t *testing.T) {
	...
	echoServer := server.(*Server)
	if len(echoServer.middleware) != 1 {
		t.Errorf("Expected 1 middleware, got %d", len(echoServer.middleware))
	}
}
...
func TestServerEventNotifications(t *testing.T) {
	...
	echoServer := server.(*Server)
	err = echoServer.observers.NotifyObservers(interfaces.EventStart, ctx, "localhost:8080")
	if err != nil {
	...
	err = echoServer.observers.NotifyObservers(interfaces.EventError, ctx, testError)
	if err != nil {
		t.Errorf("NotifyObservers error = %v, want nil", err)
	}
