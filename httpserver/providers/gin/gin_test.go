...
func TestNewFactory(t *testing.T) {
	...
	if factory.GetName() != "gin" {
		t.Errorf("Expected name 'gin', got '%s'", factory.GetName())
	}
}
...
func TestServerGetStats(t *testing.T) {
	...
	if stats.Provider != "gin" {
		t.Errorf("Expected provider 'gin', got '%s'", stats.Provider)
	}
...
func TestServerStartStop(t *testing.T) {
	factory := NewFactory()
	cfg, err := config.NewBuilder().Apply(config.WithPort(8092)).Build()
	if err != nil {
...
func TestServerHandlerWrapping(t *testing.T) {
	...
	if ginServer.routes["GET"]["/test"] == nil {
...
func TestServerWithMiddleware(t *testing.T) {
	...
	ginServer := server.(*Server)
	if len(ginServer.middleware) != 1 {
		t.Errorf("Expected 1 middleware, got %d", len(ginServer.middleware))
	}
}
...
func TestServerEventNotifications(t *testing.T) {
	...
	ginServer := server.(*Server)
	err = ginServer.observers.NotifyObservers(interfaces.EventStart, ctx, "localhost:8080")
	if err != nil {
	...
	err = ginServer.observers.NotifyObservers(interfaces.EventError, ctx, testError)
	if err != nil {
		t.Errorf("NotifyObservers error = %v, want nil", err)
	}
