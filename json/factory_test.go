...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "Stdlib Provider",
			providerType: Stdlib,
			wantNil:      false,
		},
		{
			name:         "JSONIter Provider",
			providerType: JSONIter,
			wantNil:      false,
		},
		{
			name:         "GoccyJSON Provider",
			providerType: GoccyJSON,
			wantNil:      false,
		},
		{
			name:         "JSONParser Provider",
			providerType: JSONParser,
			wantNil:      false,
		},
		{
			name:         "Default Provider for unknown type",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := New(tt.providerType)
			if (got == nil) != tt.wantNil {
				t.Errorf("New() returned %v, wantNil %v", got, tt.wantNil)
			}
		})
	}
}
...
func TestUnmarshal(t *testing.T) {
	var got testStruct
	err := Unmarshal([]byte(testJSON), &got)
	if err != nil {
...
func TestValid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := Valid([]byte(tt.jsonData)); got != tt.want {
