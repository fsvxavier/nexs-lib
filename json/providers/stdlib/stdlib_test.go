...
func TestDecoderMethods(t *testing.T) {
	...
	// Test More (since we've already read all the data, this should be false)
	if decoder.More() {
...
func TestEncoderMethods(t *testing.T) {
	...
	// The result should be prettified JSON
	output := buf.String()
	if !strings.Contains(output, "\n") {
		t.Errorf("SetIndent() did not result in multiline JSON")
	}
}
