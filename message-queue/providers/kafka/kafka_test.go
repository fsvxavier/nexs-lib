...
import (
	...
	"github.com/fsvxavier/nexs-lib/message-queue/providers/kafka/mock"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

func TestNewKafkaProvider(t *testing.T) {
	tests := []struct {
		name    string
		...
		},
		{
			...
			wantErr: false, // Kafka provider uses defaults
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := NewKafkaProvider(tt.config)
			...
			if tt.wantErr {
				...
				if provider != nil {
					assert.Equal(t, interfaces.ProviderKafka, provider.GetType())
				}
			}
		})
	}
...
func TestKafkaProvider_Connect(t *testing.T) {
	config := &config.ProviderConfig{
		Enabled: true,
		Connection: &interfaces.ConnectionConfig{
			Brokers: []string{"localhost:9092"},
	...
	// Test connect (will likely fail due to no real Kafka server)
...
func TestKafkaProvider_HealthCheck(t *testing.T) {
	config := &config.ProviderConfig{
		Enabled: true,
		Connection: &interfaces.ConnectionConfig{
			Brokers: []string{"localhost:9092"},
...
func TestKafkaProvider_Close(t *testing.T) {
	config := &config.ProviderConfig{
		Enabled: true,
		Connection: &interfaces.ConnectionConfig{
			Brokers: []string{"localhost:9092"},
...
func TestKafkaProvider_GetMetrics(t *testing.T) {
	config := &config.ProviderConfig{
		Enabled: true,
		Connection: &interfaces.ConnectionConfig{
			Brokers: []string{"localhost:9092"},
...
func TestKafkaProvider_CreateProducer(t *testing.T) {
	config := &config.ProviderConfig{
		Enabled: true,
		Connection: &interfaces.ConnectionConfig{
			Brokers: []string{"localhost:9092"},
...
func TestKafkaProvider_CreateConsumer(t *testing.T) {
	config := &config.ProviderConfig{
		Enabled: true,
		Connection: &interfaces.ConnectionConfig{
			Brokers: []string{"localhost:9092"},
...
func TestKafkaProvider_Integration(t *testing.T) {
...
// TestKafkaMockProvider testa o provider mock do Kafka
func TestKafkaMockProvider(t *testing.T) {
	mockProvider := mock.NewMockKafkaProvider()

	// Test GetType
	assert.Equal(t, interfaces.ProviderKafka, mockProvider.GetType())
...
// TestKafkaMockProducer testa o producer mock do Kafka
func TestKafkaMockProducer(t *testing.T) {
	producer := mock.NewMockKafkaProducer()
...
// TestKafkaMockConsumer testa o consumer mock do Kafka
func TestKafkaMockConsumer(t *testing.T) {
	consumer := mock.NewMockKafkaConsumer()
