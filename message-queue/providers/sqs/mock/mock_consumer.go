...
// MockSQSConsumer é um mock específico do consumer SQS
...
// NewMockSQSConsumer cria uma nova instância do mock
...
func (m *MockSQSConsumer) Subscribe(ctx context.Context, source string, options *interfaces.ConsumerOptions, handler interfaces.MessageHandler) error {
...
func (m *MockSQSConsumer) SubscribeBatch(ctx context.Context, source string, options *interfaces.ConsumerOptions, handler interfaces.BatchMessageHandler) error {
...
func (m *MockSQSConsumer) Ack(message *interfaces.Message) error {
	if m.AckFunc != nil {
...
func (m *MockSQSConsumer) Nack(message *interfaces.Message, requeue bool) error {
	if m.NackFunc != nil {
...
func (m *MockSQSConsumer) Pause() error {
	if m.PauseFunc != nil {
		return m.PauseFunc()
	}
	if m.Closed {
...
func (m *MockSQSConsumer) Resume() error {
	if m.ResumeFunc != nil {
		return m.ResumeFunc()
	}
	if m.Closed {
...
func (m *MockSQSConsumer) IsConnected() bool {
...
func (m *MockSQSConsumer) IsRunning() bool {
...
func (m *MockSQSConsumer) GetMetrics() *interfaces.ConsumerMetrics {
	...
	return &interfaces.ConsumerMetrics{
		...
		AvgProcessingLatency: time.Millisecond * 12,
