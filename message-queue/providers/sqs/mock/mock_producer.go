...
// MockSQSProducer é um mock específico do producer SQS
...
// NewMockSQSProducer cria uma nova instância do mock
func NewMockSQSProducer() *MockSQSProducer {
	return &MockSQSProducer{
		Connected: true,
		Closed:    false,
	}
}

func (m *MockSQSProducer) Publish(ctx context.Context, destination string, message *interfaces.Message, options *interfaces.MessageOptions) error {
...
func (m *MockSQSProducer) PublishBatch(ctx context.Context, destination string, messages []*interfaces.Message, options *interfaces.MessageOptions) error {
...
func (m *MockSQSProducer) PublishWithCallback(ctx context.Context, destination string, message *interfaces.Message, options *interfaces.MessageOptions, callback func(error)) error {
...
func (m *MockSQSProducer) Close() error {
	if m.CloseFunc != nil {
		return m.CloseFunc()
	}
	m.Closed = true
...
func (m *MockSQSProducer) IsConnected() bool {
...
func (m *MockSQSProducer) GetMetrics() *interfaces.ProducerMetrics {
	...
	return &interfaces.ProducerMetrics{
		...
		AvgLatency:        time.Millisecond * 25,
		LastSentAt:        time.Now(),
		MessagesPerSecond: 60.0,
	}
}
