...
// TODO: Verificar a documentação do Datadog para ver se é necessário para migração
// http://docs.datadoghq.com/tracing/trace_collection/custom_instrumentation/go/migration/
...
func StartTracing(opts ...tracer.StartOption) {
	...
	tracer.Start(opts...)
	setProvider(os.Getenv("DD_SERVICE"), os.Getenv("DD_ENV"), os.Getenv("DD_VERSION"))
}
...
func startMainSpan(operation string, startOptions ...tracer.StartSpanOption) *Span {
	if startOptions == nil {
		startOptions = []tracer.StartSpanOption{}
	}
	...
	return &Span{
		...
		traceId:      t.Context().TraceIDLower(),
		originalSpan: t,
	}
}
...
func StartMainReqSpan(req *fasthttp.Request) *Span {
	...
	if len(req.Host()) != 0 {
		startOptions = append([]tracer.StartSpanOption{
...
func SpanChildFromContext(ctx context.Context, resourceName, spanType string) (*Span, error) {
	...
	return spanChild(sc, sc.TraceIDLower(), resourceName, spanType), nil
}

// StartSpanFromContext Starts a span from context and resourceName.
func StartSpanFromContext(ctx context.Context, resourceName, spanType string) (*tracer.Span, context.Context) {
...
func spanChild(ctx ddtrace.SpanContext, traceId uint64, resourceName, spanType string) *Span {
	childStartOpts := []tracer.StartSpanOption{
	...
	return &Span{
		spanType:     spanType,
		originalSpan: tracer.StartSpan(operation).StartChild(operation, childStartOpts...),
	}
}
