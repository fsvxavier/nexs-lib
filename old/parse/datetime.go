...
// Date or Time Format Converter
// The format is used:
//   - YYYY ou yyyy for year
//   - MM ou mm for month
//   - DD ou dd for day
//   - HH ou hh for hour
//   - II ou ii for minuts
//   - SS ou ss for seconds
//
// Example of using the output format:
//
//   - YYYY-MM-DD
//   - YYYY-MM-DD HH:ii:ss
//   - yyyy-mm-dd hh:ii:ss
...
// Receive date converter in any format to a given output format
...
func ParseDateTime(datetime, format, timezone string) (string, error) {
	...
	if err != nil {
		fmt.Println("Error to parseAny date: ", err)
	}

	layout, _ := ParseFormat(regularizeDate.String())
	parsedDate, _ := time.Parse(layout, regularizeDate.String())

	return parsedDate.In(loc).Format(formatGo), err
}
