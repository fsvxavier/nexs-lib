...
func TestParseFormatToGo(t *testing.T) {
	tests := []struct {
		name   string
		input  string
		output string
...
func TestParseDateTime(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "basic date with UTC timezone",
			datetime: "2023-05-15 10:30:00",
			format:   "YYYY-MM-DD HH:II:SS",
			timezone: "",
			want:     "2023-05-15 10:30:00",
			wantErr:  false,
		},
		{
			name:     "date only format",
		...
		},
		{
			...
			want:     "2023-05-15 06:30:00",
			wantErr:  false,
		},
		{
			name:     "with empty timezone (should default to UTC)",
			datetime: "2023-05-15 10:30:00",
			format:   "YYYY-MM-DD HH:II:SS",
			timezone: "UTC",
			want:     "2023-05-15 10:30:00",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ParseDateTime(tt.datetime, tt.format, tt.timezone)
			if tt.wantErr {
				assert.NotNil(t, err)
			} else {
				assert.Nil(t, err)
