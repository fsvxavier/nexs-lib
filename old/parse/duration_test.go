...
func TestString(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		{"1us", time.Duration(time.Microsecond)},
		{"1ns", time.Duration(time.Nanosecond)},
		{"4s1ns", time.Duration(4*time.Second + time.Nanosecond)},
		{"1h4s1ns", time.Duration(time.Hour + 4*time.Second + time.Nanosecond)},
		{"1h1m10ms", time.Duration(61*time.Minute + 10*time.Millisecond)},
		{"1h1m123ms456us789ns", time.Duration(61*time.Minute + 123456789*time.Nanosecond)},
		{"1ms20ns", time.Duration(time.Millisecond + 20*time.Nanosecond)},
		{"1s20ns", time.Duration(time.Second + 20*time.Nanosecond)},
		{"693ns", time.Duration(693 * time.Nanosecond)},
		{"10s1us693ns", time.Duration(10*time.Second + time.Microsecond + 693*time.Nanosecond)},
		{"1ms1ns", time.Duration(time.Millisecond + 1*time.Nanosecond)},
		{"1s20ns", time.Duration(time.Second + 20*time.Nanosecond)},
		{"2d12h8ms", time.Duration(60*time.Hour + 8*time.Millisecond)},
		{"4d1m3s", time.Duration(96*time.Hour + 63*time.Second)},
		{"2d3s96ns", time.Duration(48*time.Hour + 3*time.Second + 96*time.Nanosecond)},
		{"1w2d3s96ns", time.Duration(168*time.Hour + 48*time.Hour + 3*time.Second + 96*time.Nanosecond)},
		{"1w2d3s3us96ns", time.Duration(168*time.Hour + 48*time.Hour + 3*time.Second + 3*time.Microsecond + 96*time.Nanosecond)},

		// this is the maximum supported by golang std: 2540400h10m10.000000000s
		{"15121w3d10m10s", time.Duration(2540400*time.Hour + 10*time.Minute + 10*time.Second)},

		// this is max int64, the max duration supported to convert
		{"15250w1d23h47m16s854ms775us807ns", time.Duration(9_223_372_036_854_775_807 * time.Nanosecond)},
